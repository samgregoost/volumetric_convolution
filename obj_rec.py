...
f_mapz = f_mapz1 + f_mapz2 + f_mapz3
...
f_mapzi = f_mapz1i + f_mapz2i + f_mapz3i
...
f_mapx = f_mapx1x + f_mapx2x + f_mapx3x
...
f_mapxxi = f_mapx1xi + f_mapx2xi + f_mapx3xi
...
f_mapxy = f_mapx1y + f_mapx2y + f_mapx3y
...
f_mapxyi = f_mapx1yi + f_mapx2yi + f_mapx3yi
...
#zi_reshape = tf.expand_dims(tf.reshape(f_mapzi, [64,1,1936])
...
#op = compact_bilinear_pooling_layer(feature_map1, feature_map2, 10000 , sum_pool=False, sequential=False)
...
caps2_output =1.0 *  tf.div(
   tf.subtract(
      caps2_output__ ,
      tf.reduce_min(caps2_output__ , keepdims = True)
   ),
   tf.subtract(
      tf.reduce_max(caps2_output__ , keepdims = True),
      tf.reduce_min(caps2_output__ , keepdims = True)
   )
)
...
for j in range(10):
	...
	#	test  = sess.run([index ], feed_dict = {y:y_annot, raw_points_init:points})
	#	print(test)
	#	print(caps2_output)
	...
	#	print(caps2_output)
		...
		#sess.run([accum_ops], feed_dict = {y:y_annot, raw_points_init:points})
			#test  = sess.run([X_ ], feed_dict = {y:y_annot, raw_points_init:points})
          	 	#print(test)
#			print(filename)
			main_itr = main_itr+1
	                print(filename)
			print(main_itr)
			loss_train, _ = sess.run([loss,training_op], feed_dict = {y:y_annot, raw_points_init:points_raw})
			print(j)
...
#				points = points_raw
				...
				#	print(rot_points)
					...
					loss_val, acc_val = sess.run(
                	        	[loss, accuracy], feed_dict = {y:y_annot, raw_points_init:points_raw})
